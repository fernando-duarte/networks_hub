...
model = Chain(Dense(N*N+N, 20, relu), Dense(20,20,relu), Dense(20, N)) |> gpu
...
opt = Descent()
@btime Flux.@epochs 50 Flux.train!(loss, ps, ncycle(train_loader, 10), opt)

test_error = mean(sum(abs2, Array(model(CuArray(x_test[:,i]))) .- p_func(x_test[1:N,i],p_bar,reshape(x_train[N+1:end,i], (N,N)),c)) for i in 1:m)
