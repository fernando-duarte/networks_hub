...
data_dict = BSON.load("data.bson") # in EC2, use the entire path: BSON.load("/home/ec2-user/networks_hub/SAN/code/network_simulations/data.bson")
...
for i=1:N
    function ff!(F, x)
        F[1] = 1-cdf(Beta(Float64(α0[i]),Float64(x[1])),Float64(w[i]/c0[i]))-Float64(delta[i]) # Distributions.jl only works with Float64
    end
    if i==1
        sol = nlsolve(ff!,[50.0])
        val = sol.zero[1]
...
# making concatenation of many A matricies for
function gen_A(N,Q)
