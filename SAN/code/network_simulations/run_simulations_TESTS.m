...
%     f = [5;5;5];
%     type = type(right_qt);
%     names = names(right_qt); %A,B,C
%     tkr = tkr(right_qt);     %A,B,C
%     beta = [NaN;NaN;NaN]; 
%     sectors = sectors(right_qt);
%     nvi_benchmark = [.025;.025;.025];
...
%     beta=nan(size(beta));    
    ...
    insur_agg = find(strcmp(sectors,'Insurance Aggregate'));
    reit_agg =find(strcmp(sectors,'REIT Aggregate'));
    other_agg =find(strcmp(sectors,'Other Aggregate'));
    ...
    [xstar,fval,exitflag,output,lambda,grad,t1] = knitro_helper(x0,Aineq,...
        ...
        uncorr_shocks);
    %can reconstruct the expected xstar using only our result
    lambda=xstar;
    xstar=[0;0;lambda/10;lambda/40;0;0;0;lambda/10;0;...
        p_bar(1)-lambda;p_bar(2)-lambda;p_bar(3)-lambda;...
        a(1)-lambda;a(2)-lambda;a(3)-lambda];
...
%     results_save_worst(end+1) = struct('qt', {q}, 'options', {fileread(file_name)}, ...
%         'exit', {exitflag}, 'times', {t1}, 'output', {xstar}, ...
%         'options_file', {file_name}, 'output_value', {output}, 'obj_value', {fval}, ...
%         'w', {w}, 'a', {a}, 'names', {names}, 'p_bar', {p_bar}, 'c', {c}, 'delta', {delta}, ...
%         'nan_c', {index_nan_c}, 'nan_b', {index_nan_b}, 'nvi', {nvi}, 'betas', {beta}, ...
%         'sectors', {sectors}, 'tkr', {tkr}, 'nvi_benchmark', {nvi_benchmark}, 'uncorr_shocks', {uncorr_shocks});
    ...
    [xstar,fval,exitflag,output,lambda,grad,t1] = knitro_helper(x0,Aineq,...
        ...
        uncorr_shocks);
    Astar_ratio_worst = reshape(xstar(1:N^2, 1),N,N);
    results_save_worst_multi(end+1) = struct('qt', {q}, 'options', {fileread(file_name)}, ...
        'exit', {exitflag}, 'times', {t1}, 'output', {xstar}, ...
        'options_file', {file_name}, 'output_value', {output}, 'obj_value', {fval}, ...
        'w', {w}, 'a', {a}, 'names', {names}, 'p_bar', {p_bar}, 'c', {c}, 'delta', {delta}, ...
        'nan_c', {index_nan_c}, 'nan_b', {index_nan_b}, 'nvi', {nvi}, 'betas', {beta}, ...
