...
BSON.load(joinpath(@__DIR__,"clearing_p_NN_gpu_sum_N$N.bson"), @__MODULE__)
...
function obj_contraction(z,p,x)
    ...
    c, α, β = selc*z, selα*z, selβ*z
    Ex = 1 .- (1/2)*sqrt.(pi ./ β).*exp.( (α .+ 1).^2 ./ (4β)).*erfc.( (α .+ 1) ./ (2sqrt.(β)) )
    return sum(c.*Ex) - mean(sump_contraction(z,p,x)) # spillovers for network
end
...
function obj_contraction_uniform(z,p,x)
    ...
    c, α, β = selc*z, selα*z, selβ*z
    Ex = 1 .- (1/2)*sqrt.(pi ./ β).*exp.( (α .+ 1).^2 ./ (4β)).*erfc.( (α .+ 1) ./ (2sqrt.(β)) )
    return sum(c.*Ex) - mean(sump_contraction(z,p,x).*extpow_prod_pdf(x,α,β)) # spillovers for network
end
...
function obj_contraction_quaduniform(z,p,x,weights)
    ...
    c, α, β = selc*z, selα*z, selβ*z
    Ex = 1 .- (1/2)*sqrt.(pi ./ β).*exp.( (α .+ 1).^2 ./ (4β)).*erfc.( (α .+ 1) ./ (2sqrt.(β)) )
    return sum(c.*Ex) - dot(sump_contraction(z,p,x).*extpow_prod_pdf(x,α,β), weights)/2^N # spillovers for network
end
...
function obj_contraction_quad(z,p,x,weigths)
    ...
    c, α, β = selc*z, selα*z, selβ*z
    Ex = 1 .- (1/2)*sqrt.(pi ./ β).*exp.( (α .+ 1).^2 ./ (4β)).*erfc.( (α .+ 1) ./ (2sqrt.(β)) )
    return sum(c.*Ex) - dot(sump_contraction(z,p,x),weigths) # spillovers for network
end
...
function int_cont!(dx,x,zp)
  ...
  prod_pdf(x) = extpow_prod_pdf(x,α,β) # betaprod_pdf(x,α,β)   # exp.(logpdf(Product(Beta.(α,β)),x))    
  dx .= (obj_x(x).*prod_pdf(x))[1,:]
end
