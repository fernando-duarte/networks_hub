...
Q = 10000 #number of samples - size(train) = Q/2 size(test) = Q/2
max_iter = 2*Q
...
y_test = zeros(1,m)
for i = 1:Q-m
    y_train[i] = p_func_true(x_train[:,i])  #x, p_bar, A, c
end
for i = 1:m
    y_test[i] =  p_func_true(x_test[:,i])
end

# Converting x to tuples
lb = zeros(N*N+3*N,1).-25
ub = ones(N*N+3*N,1).*25
x_train_init = Surrogates.sample(size(x_train,2), lb, ub, SobolSample())
x_test_init = Surrogates.sample(size(x_test,2), lb, ub, SobolSample())
for i =1:size(x_train_init,1)
    x_train_init[i] = tuple(x_train[:,i]...)
end
for i =1:size(x_test_init,1)
    x_test_init[i] = tuple(x_test[:,i]...)
end

loss(x,y) = mean((model(x).-y).^2)
evalcb() = @show(loss(x_train,y_train),loss(x_test,y_test))
model = Chain(Dense(N*N+3*N, N*3, σ), Dense(N*3,N*2,σ), Dense(N*2, N*2, σ), Dense(N*2,N*2,σ),Dense(N*2,N*2,σ), Dense(N*2,N,σ),Dense(N,N,σ), Dense(N, 1))
#@load "clearing_p_NN_gpu_sum_N$N.bson" model
init_loss = loss(x_train,y_train)
...
init_loss2 = loss(x_train,y_train)

# Training with surrogate optimize
surrogate = NeuralSurrogate(x_train_init,vec(y_train),vec(lb),vec(ub),model = model,loss = loss ,opt=opt,n_echos=2)
surrogate_optimize(p_func_true,SRBF(),lb,ub,surrogate,UniformSample())
@show(loss(x_train,y_train))
@show(loss(x_test,y_test))

model = cpu(model)
@save "clearing_p_NN_gpu_sumsur_N$N.bson" model
model = gpu(model)
## Testing
