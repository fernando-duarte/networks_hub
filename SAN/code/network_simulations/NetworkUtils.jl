...
struct optimParam{T}
    hᵀ::Union{SparseMatrixCSC{T, Int64},Matrix{T}}
    q::Vector{T}
    H::Union{SparseMatrixCSC{T, Int64},Matrix{T}}
    M::Integer
    net::Network 
    mini_batch::Integer
    selA::Union{SparseMatrixCSC{T, Int64},Matrix{T}}
    selb::Union{SparseMatrixCSC{T, Int64},Matrix{T}}
    selc::Union{SparseMatrixCSC{T, Int64},Matrix{T}}
    selα::Union{SparseMatrixCSC{T, Int64},Matrix{T}}
    selβ::Union{SparseMatrixCSC{T, Int64},Matrix{T}}
    selp::Union{SparseMatrixCSC{T, Int64},Matrix{T}}
    getA::Any
    getp::Any
    vars::Any
    #inner constructor    
    function optimParam(network::Network{T,N}, mini_batch::Integer=1; dense::Bool=false) where {T,N}
        ...
        if !dense
            ...
            new{T}(Array(hᵀ),q,Array(H),M,network,mini_batch,Array(selA),Array(selb),Array(selc),Array(selα),Array(selβ),Array(selp),getA,getp,vars)
        end
    end
end #struct
...
# p(A,c,b,x) approximated by the neural network called `model`
function p_NN(z,x,NN)
    model(vcat(z[1:N^2+2N],x))
end
...
function obj(z,x,op::optimParam; use_nlsolve::Bool=false)
    @unpack selc, selα, selβ, getA, getp, net, mini_batch = op 
    @unpack p_bar, γ = net
    Aᵀ = transpose(getA(z))
    c = selc*z
    α = selα*z
    ...
    if use_nlsolve
        function contraction!(F, p, x, Aᵀ, c,γ,p_bar_rep,c_rep)
            F .= min.(p_bar_rep,max.( (1+γ)*(Aᵀ*p + (one(x[1]) .- x).*c_rep) - γ.*p_bar_rep, 0 ) )
        end
        contraction!(F,p) = contraction!(F, p, x, Aᵀ, c,γ,p_bar_rep,c_rep)
        function clearing_p!(F, p)
            F .= p - contraction!(F,p)
        end
        p = nlsolve((F, p)-> clearing_p!(F, p),p_bar_rep, autodiff = :forward).zero
    else
        function contraction!(p, x, Aᵀ, c,γ,p_bar_rep,c_rep)
            return min.(p_bar_rep,max.( (1+γ)*(Aᵀ*p + (one(x[1]) .- x).*c_rep) - γ.*p_bar_rep, 0 ) )
        end
        contraction_iter!(x, n::Integer) = n <= 0 ? F  : contraction!(contraction_iter!(F,x,n-1))
        p = contraction_iter!(x,10)
    end
    
    return sum(c.*x)-sum(mean(p,dims=2)) # spillovers for network 
