...
data = data[1:15,:] # keep small number of nodes, for testing
...
temp = convert(Array{Float64},data[:,[:delta, :delta_alt, :w, :assets, :p_bar]])
delta = copy(temp[:,1]); 
...
plow = (Matrix{Float64}(I,N,N)-Matrix{Float64}(I,N,N)/2)*(Matrix{Float64}(I,N,N)-Asol0)\csol0
plow.<=p_bar
sum(-min.(plow,p_bar))
sum(-psol0)
...
sum(x0.+p_bar.-psol0) 
loss(x0,Asol0)
sum(x0.+p_bar.-min.(plow,p_bar)) 
loss_d(x0)
sum(x0.+p_bar.-min.(plow,p_bar)) /loss_d(x0)
loss(x0,Asol0)/loss_d(x0)
