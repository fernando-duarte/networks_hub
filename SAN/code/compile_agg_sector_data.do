...
*Separate locals for mkmv_id samples in EACH quarter. So firms are included for quarters when not in Y9C. 
...
local tags insurance dealer reit other
foreach tag in `tags'{
	global `tag'_vars
...
preserve
	...
	*Average default prob by assets, using V9 and V8 of KMV EDF. Will sum these across quarters
	...
	gen prob_sum = edf01 * (total_assets_current_amt/assets_total)
	gen prob_sum_8 = edf01_8 * (total_assets_current_amt/assets_total)
	...
	*Nice for troubleshooting
	...
	collapse (sum) to_reduce_ass to_reduce_liab prob_sum prob_sum_8 (lastnm) assets_total, by(qt_dt name)
	format qt_dt %tq
	...
	*	later on
	...
	save ../temp/broker_nodes.dta, replace
...
foreach tkr in $`tag'_kmv{
	foreach qt in `y9c_quarters'{
		if strpos(" `q_`qt'' ", " `tkr' "){
			//if !inlist("`tag'", "other"){
			//	replace total_`tag' = total_`tag' - weights`tkr'/1000 if qt_dt == `qt'
			//	replace liab_total_`tag' = liab_total_`tag' - total_liabilities_amt`tkr'/1000 if qt_dt == `qt'
			//}
...
preserve
	...
	keep if _merge == 3
	append using ../temp/broker_nodes_list
...
*Output
drop edf* weights* full_weights* test_perc*
